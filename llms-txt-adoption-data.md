...
## What the same data points to instead

The things that keep showing up in adoption and citation data instead of
llms.txt: whether crawlers can actually reach the content in the first place
(a shocking number of sites accidentally block the exact bots they want),
real schema markup, and content that directly answers a question instead of
circling it. All slower to fix than dropping a text file at the root. All
showing up in the actual data as the parts that correlate with citation.

## So I didn't add it

Neither of my sites has an `llms.txt` file, and neither is getting one on the
strength of this. Something with roughly 10% adoption and no measurable
relationship to citation isn't worth shipping, even at five minutes of
effort. If the things I'm prioritizing instead — crawler access, real schema,
answering the actual question — don't move anything either on either site,
that's the next thing I'll log here.
